...
class GetOrdersFor3Days extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      isLoaded: false,
  ...
  componentDidMount() {
    ...
    if (window.location.hostname === "localhost") {
      fetch('http://localhost:49934/api/Orders3Days')
      ...
      fetch('http://proj.ruppin.ac.il/bgroup81/prod/api/Orders3Days')
        .then(this.handleErrors)
        .then(response => response.json())
        .then(data => {
          this.setState({
    ...
    if (this.state.orders.length === 0) {
      return (
        <div className="divtbl">          
          <h5>לא נמצאו הזמנות לשלושת הימים הקרובים</h5>
          <hr></hr>
        </div>)
    }
    else {
...
export default GetOrdersFor3Days;